...
/**
 * subscribe for live updates and register callback 
 * 
 * @param {string} storeEndPoint The datastore uri
 * @param {string} driverName The name of the driver registering the datasource 
 * @param {string} datasourceName The name of the datasource (must be avalable in the datastore)
 * @param {function} onDataCallback A function to callback when new data is available.
 * @returns
 */
module.exports.subscribeToDatasource = function (storeEndPoint, driverName, datasourceName, onDataCallback) {
  
  return new Promise((resolve, reject) => {
  
    
    openWS(storeEndPoint)
    .then(() => {
      
      //register callback
      wsCallbacks[datasourceName] = onDataCallback;
      
      //SUB for data
      var options = {
        uri: storeEndPoint+'/sub/'+datasourceName,
        method: "GET"
      };
      console.log("[WS] trying to subscribe for updates.", options);
      databoxRequest(options,(error,response,data)=>{
        if (error) {
          console.log("[ERROR] Can not register datasource subscription with datastore!", error);
          reject(error);
          return;
        } else if(response && response.statusCode != 200) {
          console.log("[ERROR] Can not register datasource subscription with datastore!", body);
          reject(body);
          return;
        }
        console.log("[WS] subscribed for updates for " + datasourceName);
        resolve();
      });
    })
    .catch((err)=>{
